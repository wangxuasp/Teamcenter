/*HEAD LIST_ITEM_REVISIONS_ACLS CCC ITK */
#include <stdlib.h>
#include <time.h>
#include <ae/ae.h>
#include <sa/am.h>
...
static void do_it(void)
{
    ...
    printf("Number of lines in the effective ACL = %d\n", length);
    ...
    for (ii = 0; ii < length; ii++)
    {
        ...
        for (kk = 0; kk < n_granted; kk++)
        {
            IFERR_REPORT(AM_ask_priv_name(granted[kk], &privilege_name));
            printf("  %s - GRANTED\n", privilege_name); 
        }
            
        for (kk = 0; kk < n_revoked; kk++)
        {
            IFERR_REPORT(AM_ask_priv_name(revoked[kk], &privilege_name));
            printf("  %s - REVOKED\n", privilege_name);  
        }
