 ...
 * @file 
 *
 *   This file contains the implementation for the Extension A2validateSaveAsInput
 *
 */
#include <A2irs/A2validateSaveAsInput.hxx>
...
int A2validateSaveAsInput( METHOD_message_t * msg, va_list args )
{
    int ifail = ITK_ok; 

    va_list largs;
    va_copy( largs, args );
    Teamcenter::SaveAsInput *saveAsInput = va_arg(largs, Teamcenter::SaveAsInput*);
    ...
    saveAsInput->getTag("newTargetObject", newTargetObject, isNull);

    tag_t srcObjectTag = NULLTAG;
    saveAsInput->getTag("srcObjectTag", srcObjectTag, isNull);
   
    bool performDeepCopy = true;
    saveAsInput->getLogical( "performDeepCopy", performDeepCopy, isNull);
    
	tag_t item_master_tag = NULLTAG;
    saveAsInput->getTag("item_master_tag", item_master_tag, isNull);

    std::string revid;
    saveAsInput->getString("item_revision_id", revid, isNull);
