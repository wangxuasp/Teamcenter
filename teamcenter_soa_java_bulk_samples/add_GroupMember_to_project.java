...
import com.teamcenter.soa.client.model.strong.User;
	...
	public void addMemberToProject(String userId, String groupName, String roleName, String projectId, int memberType)
	{ 
		...
		int newRegLen = regLen + 1; // regular members increases in all cases
		int newPrivLen = privLen;
		int newAdminLen = adminLen;
		
		if(memberType == MEMBER_TYPE_PRIVILEGED) newPrivLen = newPrivLen + 1;
		if(memberType == MEMBER_TYPE_TEAM_ADMIN) newAdminLen = newAdminLen + 1;		
		int totalLength = newRegLen + newPrivLen + newAdminLen;
		...
		memberInfo[index] = new TeamMemberInfo();
		memberInfo[index].teamMember = groupMember;
		memberInfo[index].teamMemberType = MEMBER_TYPE_REGULAR;
		index++;
		...
		if(memberType == MEMBER_TYPE_PRIVILEGED)
		{
			memberInfo[index] = new TeamMemberInfo();
			memberInfo[index].teamMember = user;
			memberInfo[index].teamMemberType = MEMBER_TYPE_PRIVILEGED;
			index++;
		}
		...
		if(memberType == MEMBER_TYPE_TEAM_ADMIN)
		{
			memberInfo[index] = new TeamMemberInfo();
			memberInfo[index].teamMember = user;
			memberInfo[index].teamMemberType = MEMBER_TYPE_TEAM_ADMIN;
